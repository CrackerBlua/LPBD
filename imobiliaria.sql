...
(7, '7', 'Braga Avenida - Israelândia', '083', '57127-227', 'Bom Retiro', 'Rio Grande', 'MS', 'Disponível', 'Sala comercial', 'Locação', null, 86301641582, null, 0, 0, 2, 0, 3, 60, false, 'Salão de beleza', 0, 0, false, 2, 4, 12, 5), 
...
CREATE TABLE IF NOT EXISTS transacao_imovel ( 
	id INT NOT NULL AUTO_INCREMENT,
    id_imovel VARCHAR(70) REFERENCES imoveis(id_imoveis) ON UPDATE CASCADE,
...
CREATE VIEW Vendas_imoveis AS SELECT * FROM imoveis WHERE tipo_negocio = 'Venda'; #1

CREATE VIEW Locacao_imoveis AS SELECT * FROM imoveis WHERE tipo_negocio = 'Locação'; #2

SELECT * FROM transacao_imovel WHERE feita_transicao = TRUE AND valor_transacao > 300000; #3

SELECT clientes.id, clientes.nome, clientes.cpf, clientes.telefone, clientes.relacao_imovel, imoveis.status_imovel, imoveis.tipo_imovel, imoveis.tipo_negocio FROM clientes LEFT JOIN imoveis ON clientes.cpf = imoveis.cpf_proprietario; #4 

USE imobiliaria_bd;
SELECT * FROM imoveis WHERE status_imovel = 'Indisponível' ORDER BY tipo_imovel; #5

USE imobiliaria_bd;
SELECT * FROM imoveis WHERE status_imovel = 'Disponível' ORDER BY tipo_imovel; #6

USE imobiliaria_bd;
SELECT * FROM documentos WHERE nome like '%Loca%' ORDER BY tipo_documento; #7

USE imobiliaria_bd;
SELECT * FROM documentos WHERE nome like '%Vende%' ORDER BY tipo_documento; #8

USE imobiliaria_bd;
SELECT * FROM transacao_imovel WHERE tipo_transacao = 'Venda' AND valor_sugerido_proprietario >= (SELECT AVG(valor_sugerido_proprietario) FROM transacao_imovel); #9

USE imobiliaria_bd;
SELECT * FROM transacao_imovel WHERE tipo_transacao = 'Alocado' AND valor_sugerido_proprietario >= (SELECT AVG(valor_sugerido_proprietario) FROM transacao_imovel WHERE tipo_transacao = 'Alocado'); #10
